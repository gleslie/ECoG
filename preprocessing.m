...
eeglab_dir = '/Users/robertjquon/Desktop/sound_study1/ECoG/dependencies/eeglab14_1_2b';
data_dir = '/Users/robertjquon/Desktop/sound_study1/Downsampled';
end_data_dir = '/Users/robertjquon/Desktop/sound_study1/processed';
timestamps_filename = 'finalLocs_current.mat';
ImportDataSetNumber = 6; %% Change this to the dataset in the list you want to import (refer to 's').      %%%%%%%%%%
...
tablesounds11 = load('-mat', timestamps_filename);
% Create new variables in the base workspace from those fields.
vars = fieldnames(tablesounds11);
for i = 1:length(vars)
    assignin('base', vars{i}, tablesounds11.(vars{i}));
end
tablesounds11 = tablesounds11.finalLocs;
...
% removing non ECoG channel data        %%%%%%%%%% DO THIS MANUALLY WITH EEGLAB (edit->sel data)
...
% trial_data = cell(length(tablesounds11),1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Find all rows with this subject's data
[filepath,subject_name,ext] = fileparts(EEGfiles{ImportDataSetNumber})
index = find(strcmp(tablesounds11(:,1),subject_name));

for it = 1:length(index)
    idx1 = cell2mat(tablesounds11(index(it),3));
    idx2 = cell2mat(tablesounds11(index(it),4));
    trial_data{index(it)} = EEG.data(:,idx1:idx2);
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

save([end_data_dir '/trial_data_ss2'], 'trial_data', '-v7.3')
